(function (name, definition) {
  var hasDefine = typeof defines == "function",
    hasModuleExports = typeof moduled != "undefined" && moduled.exports;

  //当存在defines、moduled函数时候，包装为模块
  if (hasDefine) {
    defines(name, definition);
  } else if (hasModuleExports) {
    moduled.exports = definition();
  } else {
  ...
  _u.success = function (instence, info, duration) {
  ...
  _u.warning = function (instence, info, duration) {
  ...
  _u.error = function (instence, info, duration) {
  ...
  //小数和整数的正则表达式
  _u.pattern=/^\d+(\.{0,1}\d+){0,1}$/;

  //正整数的正则表达式
  _u.pattern1=/^[0-9]*[1-9][0-9]*$/;
  ...
  _u.getOrgTreeEscrow = function (that,oid,otype, callback) {
    var url = api.sysOrg_tree;
    var orgType = otype;
    if (orgType == 'seaStation') {
      url = api.escrow_tree;
    }
    // var data = {id: 1};
    var data = {id: oid};
    that._req.post(data, url, function (_r, _x) {
      if (_r.success) {
        callback(_r.data);
      } else {
        _u.error(that, "单位数据获取失败" + _r.message);
      }
    }, function (_ex) {
      _u.error(that, "单位数据获取失败" + JSON.stringify(_ex));
    });
  };

  _u.getOrgList = function (that, callback) {
    var url = api.orgList;
    that._req.post({}, url, function (_r, _x) {
      if (_r.success) {
        callback(_r.data);
      } else {
        _u.error(that, "单位列表数据获取失败" + _r.message);
      }
    }, function (_ex) {
      _u.error(that, "单位列表数据获取失败" + JSON.stringify(_ex));
    });
  };
  ...
  _u.getRoles = function (vueApp, callback) {
    ...
    vueApp._req.post(vueApp, data, url, function (_r, _x) {
      if (_r.success) {
  ...
  _u.buildFormData = function (data) {
  ...
  _u.datetimeToString = function (datetime) {
  ...
  /*字符串转时间戳*/
  _u.stringToTimestamp = function (date) {
  ...
  ** → "2322年02月06日 03时45分"
  */
  _u.dateFormat = function (timestamp, formats) {
    // formats格式包括
    // 1. Y-m-d
    // 2. Y-m-d H:i:s
    // 3. Y年m月d日
